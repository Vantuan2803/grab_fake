...
class adminwrk
{
    ...
    function alocation($location, $distance, $available, $conn)
    {
        ...
        if ($result->num_rows > 0) {
            $errors[] = array('input' => 'result', 'msg' => 'Tên người dùng đã tồn tại');
            echo '<p class="bg-danger text-center">Địa điểm đã tồn tại</p>';
        }
        if (count($errors) == 0) {
            $sql = "INSERT INTO location(`name`, `distance`, `is_available`) VALUES('" . $location . "', '" . $distance . "', '" . $available . "')";

            if ($conn->query($sql) === true) {
                echo '<p class="bg-success text-center">Đã thêm địa điểm thành công</p>';
            } else {
                echo '<p class="bg-danger text-center">Đã xảy ra sự cố</p>';
            }
        }
    }
    ...
    function book($pickup, $drop, $cabtype, $dist, $far, $lugg, $datetime, $id, $driver_id, $conn)
    {
        var_dump($pickup);
        echo $sql = "INSERT INTO ride(`ride_date`,`from_distance`,`to_distance`,`cab_type`,`total_distance`,`luggage`,`total_fare`,`status`,`customer_user_id`, `driver_id`) VALUES('" . $datetime . "','" . $pickup . "','" . $drop . "','" . $cabtype . "','" . $dist . "','" . $lugg . "','" . $far . "',1,'" . $id . "', '".$driver_id."')";
    ...
    function elocation($location, $distance, $available, $id, $conn)
    {
        ...
        if (count($errors) == 0) {
            ...
            if ($conn->query($sql) === true) {
                echo '<p class="bg-success text-center">Chỉnh sửa địa điểm thành công</p>';
            } else {
                echo '<p class="bg-danger text-center">Đã xảy ra sự cố</p>';
            }
        }
    ...
    {
        $sql = "SELECT * FROM user WHERE role = 3 AND isblock=0";
        $result = $conn->query($sql);
        $appr = array();
        while ($row = $result->fetch_assoc()) {
            array_push($appr, $row);
    ...
    function aproveDriver($conn)
    {
        $sql = "SELECT * FROM user WHERE role = 2 AND isblock=0";
        $result = $conn->query($sql);
        $appr = array();
        while ($row = $result->fetch_assoc()) {
            array_push($appr, $row);
    ...
    {
        $sql = "SELECT * FROM user WHERE role = 3 AND isblock=1";
        $result = $conn->query($sql);
        $appr = array();
        while ($row = $result->fetch_assoc()) {
            array_push($appr, $row);
    ...
    function aprovedDriver($conn)
    {
        $sql = "SELECT * FROM user WHERE role = 2 AND isblock=1";
        $result = $conn->query($sql);
        $appr = array();
        while ($row = $result->fetch_assoc()) {
            array_push($appr, $row);
